...
namespace RobloxDeployHistory
{
    public class StudioDeployLogs
    {
        ...
        public static async Task<DeployLog> GetVersionData(Channel channel, string Version = "Latest")
        {
            ...
            HashSet<DeployLog> targets = Environment.Is64BitOperatingSystem ? logs.CurrentLogs_x64 : logs.CurrentLogs_x86;

            if (logs.HasHistory)
            {
                List<DeployLog> input = targets.OrderByDescending(log => log.TimeStamp).Cast<DeployLog>().ToList();
